...
13. **Page zoom logic lives exclusively in `PageZoomController.kt` and must use only WebView's native `zoomIn()`/`zoomOut()`/`zoomBy()` API — never `textZoom`, `setInitialScale()`, or custom CSS/JS zoom injection. Do not move this logic elsewhere or reimplement it inline in `WebViewEx.kt`/`WebViewWebEngine.kt` without explicit instruction to do so.**
...
| 18 | Page zoom regressed a second time using `WebSettings.textZoom` and `WebView.setInitialScale()` | Attempting to fix CSS zoom by using `textZoom` (which only scales font size, ignoring buttons/images/layout) and `setInitialScale()` (a one-time render hint that conflicts with `<meta name="viewport">`) broke zoom parity and layout | Isolated all zoom logic into dedicated protected file `PageZoomController.kt` using exclusively WebView's native page-zoom API (`zoomIn()`, `zoomOut()`, `zoomBy()`), with per-tab native scale restoration | ✅ Fixed |
